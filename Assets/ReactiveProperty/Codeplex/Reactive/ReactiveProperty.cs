...
namespace Codeplex.Reactive
{
    ...
    public class ReactiveProperty<T> : IObservable<T>, IDisposable, INotifyPropertyChanged, IReactiveProperty
    {
            ...
            ReactivePropertyMode mode = ReactivePropertyMode.DistinctUntilChanged|ReactivePropertyMode.RaiseLatestValueOnSubscribe)
        {
            ...
            // create source
            var merge = Observable.Merge(source, anotherTrigger);
            if ((mode & ReactivePropertyMode.DistinctUntilChanged) != 0) merge = merge.DistinctUntilChanged();
            merge = merge.Do(x =>
            {
            ...
            // publish observable
            var connectable = (mode & ReactivePropertyMode.RaiseLatestValueOnSubscribe) != 0
        ...
        public ReactiveProperty<T> SetValidateNotifyError(Func<IObservable<T>, IObservable<IEnumerable>> validator)
        {
            ...
            this.validateNotifyErrorSubscription.Disposable
                ...
                .Select(xs =>
                {
                    ...
                    return strings.Cast<object>().Concat(others);
                })
                .Subscribe(x =>
                {
        ...
        {
            return this.SetValidateNotifyError((IObservable<T> xs) => validator(xs).Cast<string, IEnumerable>());
        }
